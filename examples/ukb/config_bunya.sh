#!/bin/bash
# ==========================================================================
# Bunya HPC configuration for UKB RISH-GLM pipelines
#
# Overrides local paths from config.sh for the Bunya cluster.
# Sources shared helper functions from config.sh after overriding paths.
#
# QSIPrep output: /scratch/user/uqahonne/ukb/bed_analysis/qsiprep_output_bed_controls_recovered
# Project space:  /scratch/user/uqahonne/ukb/bed_analysis/rish_harmonize
# ==========================================================================

# Load Neurodesk MRtrix3 (replaces conda mrtrix3_env)
module purge
...
# rish-harmonize CLI (install into a venv on Bunya)
RISH_VENV="/scratch/user/uqahonne/ukb/bed_analysis/rish_harmonize/venv"
if [[ -f "$RISH_VENV/bin/activate" ]]; then
    source "$RISH_VENV/bin/activate"
fi

# Override paths for Bunya
...
declare -A SITE_MAP=(
    ...
    [sub-0000007]=11025
    [sub-0000008]=11027 [sub-0000009]=11027 [sub-0000010]=11027
    [sub-0000011]=11026 [sub-0000012]=11026 [sub-0000013]=11026
    [sub-0000014]=11028 [sub-0000015]=11028
)
SUBJECTS=(${!SITE_MAP[@]})
SUBJECTS=($(printf '%s\n' "${SUBJECTS[@]}" | sort))
...
# ==========================================================================
# Shared steps (same logic as local config.sh)
# ==========================================================================
step_convert_dwi() {
    local OUT=$1
    echo ""
...
step_verify() {
    ...
    if [[ -f "$GLM_DIR/glm/shell_lmax.json" ]]; then
        ...
        echo "  [FAIL] Looks like the shell_lmax.json missing"
    fi
        ...
        if [[ -f "$SITE_DIR/scale_maps_meta.json" ]]; then
            ...
            for SCALE_L0 in "$SITE_DIR"/b*/scale_l0_*.mif; do
                if [[ -f "$SCALE_L0" ]]; then
                    ...
                    MEAN=$(mrstats "$SCALE_L0" -mask "$MASK" -output mean 2>/dev/null)
                    MEDIAN=$(mrstats "$SCALE_L0" -mask "$MASK" -output median 2>/dev/null)
                    STD=$(mrstats "$SCALE_L0" -mask "$MASK" -output std 2>/dev/null)
